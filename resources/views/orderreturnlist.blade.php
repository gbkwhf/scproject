...
@section('contentheader_title','订单退款')
          ...
          <div class="box">
               ...
               <br><br>
                  <div class="box-tools2 ">
                      <form class="form-horizontal" id ="form_action" action="{{url('manage/returnorder')}}" method="get">
                          <div style="width: 800px;" class="input-group input-group-sm row">
                              ...
                              <div class="col-lg-2">
	                            <select name="state"  class="form-control pull-right"  style="width: 135px">
	                                ...
	                                <option @if(isset($_GET['state'])) @if($_GET['state'] == 1) selected="selected" @endif @endif  value=1>已通过</option>
                                    <option @if(isset($_GET['state'])) @if($_GET['state'] == 2) selected="selected" @endif @endif  value=1>已拒绝</option>
	                            </select>
                              </div>                                                                                                                                             
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                <tbody><tr><!-- 用户名 手机号，订单号，总金额，付款时间，状态-->
                  <th>用户名</th>
                  <th>手机号</th>
                  <th>订单号</th>
                   <th>总金额</th>
                ...
                @foreach ($data as $bill)    				
	    			<tr>
	    			  <td>{{ $bill->name }}</td>
	                  <td>{{ $bill->mobile }}</td>
                        <td>{{ $bill->base_order_id }}</td>
	                  <td>{{ $bill->amount }}</td>
                      <td>{{ $bill->created_at }}</td>
                        <td>{{ $bill->state }}</td>
	                  <td>
	                  		<a href="{{ url('manage/returnorderdetial',['id'=>$bill->id]) }}"><button class="btn bg-orange margin" type="button">详情</button></a>
	                  </td>
	                </tr>                
				@endforeach               
              </tbody></table>
