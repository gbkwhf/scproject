...
@section('contentheader_title','商品类型列表')
             ...
             {{--<div class="box-header">--}}
               {{--<br><br>--}}
                  {{--<div class="box-tools2 ">--}}
                      ...
                      {{--</form>--}}
                  {{--</div>--}}
                   <div style="float:right;margin-top: -55px;"><a href="{{url('goods/type/add')}}">  <button type="button" class="btn bg-olive margin" >添加商品类型</button></a></div>
              </div>
                <!-- /.box-header -->
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                        ...
                        @foreach ($data as $goods_type)
                            <tr>

                                <td>{{ $goods_type->id }}</td>
                                <td>{{ $goods_type->goods_type_name }}</td>
                                <td style="width: 600px;">
                                    @if(!empty($goods_type->others))

                                         @foreach($goods_type->others as $k=>$v)
                                ...
                                <td>{{ $goods_type->sort }}</td>
                                <td>
                                    <a href="{{ url('goods/type/edit',['id'=>$goods_type->id]) }}"><button class="btn bg-orange margin" type="button">编辑</button></a>
                                    <a href="javascript:if(confirm('确实要删除吗?'))location='{{ url('goods/type/delete',['id'=>$goods_type->id]) }}'"><button class="btn bg-maroon margin" type="button">删除</button></a>
                                </td>
                                </td>
                            </tr>
                        @endforeach
