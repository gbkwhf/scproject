...
@section('main-content')
    ...
    <div class="box box-info">
        ...
        @section('contentheader_title','编辑商品类型')

        <form class="form-horizontal" action="{{url('goods/type/edit/save')}}" method="post"  enctype ="multipart/form-data">


                    <div class="form-group">
                        <label class="col-sm-2 control-label" >商品类型名称</label>
                        <div class="col-sm-10">
                            <input type="text"  name="goodsTypeName"  placeholder="Enter ..." class="form-control"  value="{{$data->type_name}}" >
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            ...
                            @if(!empty($spec_all))
                                @foreach($spec_all as $k=>$v)

                                    @if(in_array($v->id,$spec))
                                        <input type="checkbox" name="spec" value="{{$v->id}}" checked="checked" />{{$v->name}}
                                    @else
