...
@section('main-content')
    ...
    <div class="box box-info">
        <!-- /.box-header -->
        <!-- form start -->
        @section('contentheader_title','编辑商品分类')

        <form class="form-horizontal" action="{{url('commody/class/manage/edit/save')}}" method="post"  enctype ="multipart/form-data">


                    <div class="form-group">
                        <label class="col-sm-2 control-label" >分类名称</label>
                        <div class="col-sm-10">
                            <input type="text"  name="class_name"  placeholder="Enter ..." class="form-control" value="{{$data->name}}">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="first_id_value"  class="col-sm-10"  style="width: 135px;height:30px; margin-left: 15px;">
                            ...
                            @foreach($first_info as $tmp)
                                <option @if($tmp->id == $data->first_id) selected @endif   value="{{$tmp->id}}">{{$tmp->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="goods_type"  class="col-sm-10"  style="width: 135px;height:30px;margin-left: 15px;">
                            @foreach($type_info as $arr)
                                <option @if($arr->id == $data->goods_type) selected @endif   value="{{$arr->id}}">{{$arr->name}}</option>
                            @endforeach
                        </select>
                    </div>
