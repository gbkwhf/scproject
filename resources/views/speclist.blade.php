...
@section('contentheader_title','商品规格列表')
             ...
             {{--<div class="box-header">--}}
               {{--<br><br>--}}
                  {{--<div class="box-tools2 ">--}}
                      ...
                      {{--</form>--}}
                  {{--</div>--}}
                   <div style="float:right;margin-top: -55px;"><a href="{{url('spec/add')}}">  <button type="button" class="btn bg-olive margin" >添加商品规格</button></a></div>
              </div>
                <!-- /.box-header -->
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach ($data as $spec)
                            <tr>
                                ...
                                <td>{{ $spec->name }}</td>
                                <td>
                                    <a href="{{ url('spec/edit',['id'=>$spec->id]) }}"><button class="btn bg-orange margin" type="button">编辑</button></a>
                                    <a href="javascript:if(confirm('确实要删除吗?'))location='{{ url('spec/delete',['id'=>$spec->id]) }}'"><button class="btn bg-maroon margin" type="button">删除</button></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
