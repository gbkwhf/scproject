...
class SendToUserSMSController  extends  Controller{
    ...
    public function sendToUserSMS(Request $request){
        ...
        $validator = $this->setRules([
            ...
            'service_type' => 'required|integer|in:1,3,4,5,g6', //1注册   2邮箱找回密码  3手机重置密码    4.短信登陆（免密登陆）  5绑定openId  6邀请注册
        ])
        ...
        switch($request->get('service_type')){
            ...
            case 5:
                $message = "【双创共享】感谢您使用双创共享！您的验证码为:[XXX]，10分钟内有效。"; //绑定openId
                $state = $this->SendSMSAll($request->get('un'),$request->get('service_type'),$message);
                break;
   ...
   //（1）处理注册发短信业务逻辑   ---->处理手机号码不在系统内的情况
    ...
    //（2）处理免密登陆和重置密码业务逻辑    ------>处理手机号码在系统内的情况
    ...
    //（3）处理绑定微信发验证码  --->这个方法不验证手机号码是否存在系统内（即是否手机号码在系统内不重要）
    private function SendSMSAll($mobile,$type,$message){
